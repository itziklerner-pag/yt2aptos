 ...
 * Token storage keys
 */
const TOKEN_STORAGE_KEY = 'aptosfs_auth_tokens';
const USER_STORAGE_KEY = 'aptosfs_user';
...
class AuthService {
  ...
  public async requestPasswordReset(email: string): Promise<{ message: string }> {
    ...
    const response = await apiClient.post<{ message: string }>('/auth/password-reset/request', request);
    return response.data;
  }
  ...
  public async resetPassword(resetToken: string, newPassword: string): Promise<{ message: string }> {
    ...
    const response = await apiClient.post<{ message: string }>('/auth/password-reset/reset', request);
    return response.data;
  }
