...
import { DownloadJob, DownloadJobDocument, DownloadJobStatus } from '../download-job.model';
...
describe('DownloadJob Model', () => {
  ...
  let testVideoId: mongoose.Types.ObjectId;

  // Valid status values
  const validStatuses: DownloadJobStatus[] = [
    'queued', 
    'processing', 
    'completed', 
    'failed',
  ...
  beforeAll(async () => {
    ...
    testVideoId = savedVideo._id as mongoose.Types.ObjectId;
  });

  // Clean up after tests
  afterAll(async () => {
    await DownloadJob.deleteMany({});
  ...
  it('should require videoId and userId fields', async () => {
    ...
    const jobWithoutVideoId = new DownloadJob({
      userId: testUserId,
    });

    // Test without userId
    const jobWithoutUserId = new DownloadJob({
      videoId: testVideoId,
    });
  ...
  it('should only allow valid enum values for status', async () => {
    ...
    // Test invalid status
    const jobWithInvalidStatus = new DownloadJob({
      videoId: testVideoId,
      userId: testUserId,
      status: 'invalid' as any, // Intentionally testing an invalid value
    });
  ...
  it('should query jobs by status and priority', async () => {
    ...
    const processingJobs = await DownloadJob.find({ status: 'processing' });
  ...
  it('should find jobs by user ID', async () => {
    ...
    const userJobs = await DownloadJob.find({ userId: testUserId });
    ...
    const otherUserJobs = await DownloadJob.find({ userId: savedAnotherUser._id });
    expect(otherUserJobs.length).toBe(1);
  });

  it('should find jobs by video ID', async () => {
    // Create another video
    const anotherVideo = new Video({
    ...
    const videoJobs = await DownloadJob.find({ videoId: testVideoId });
    ...
    const otherVideoJobs = await DownloadJob.find({ videoId: savedAnotherVideo._id });
  ...
  it('should support searching with various criteria combinations', async () => {
    ...
    const completedJobs = await DownloadJob.find({
      status: 'completed',
      progress: 100,
    });
