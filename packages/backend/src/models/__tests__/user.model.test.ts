...
describe('User Model', () => {
  ...
  it('should require username, email and password', async () => {
    const userWithoutUsername = new UserModel({
      email: 'test@example.com',
  ...
  it('should set default values', async () => {
    const userWithoutOptionalFields = new UserModel({
      username: 'minimalist',
  ...
  it('should save and retrieve user correctly with typed interface', async () => {
    ...
    const user = new UserModel(typedUser);
    await user.save();
    
    // Retrieve the user
    const retrievedUser = await UserModel.findOne({ email: typedUser.email });
