...
describe('Playlist Model', () => {
  ...
  it('should require youtubeId, title, channelId, and createdBy fields', async () => {
    ...
    const playlistWithoutYoutubeId = new Playlist({
    ...
    // Test without title
    const playlistWithoutTitle = new Playlist({
    ...
    const playlistWithoutChannelId = new Playlist({
      youtubeId: 'PL67890fghij',
      title: playlistData.title,
      createdBy: testUserId,
    });
    ...
    const playlistWithoutCreatedBy = new Playlist({
      youtubeId: 'PL67890fghij',
      title: playlistData.title,
      channelId: testChannelId,
    });
  ...
  it('should not allow duplicate youtubeId', async () => {
    ...
    const duplicatePlaylist = new Playlist({
      youtubeId: 'PL98765zyxwv',
  ...
  it('should set default values correctly', async () => {
    const playlistWithDefaults = new Playlist({
  ...
  it('should only allow valid enum values for archiveStatus', async () => {
    ...
    // Test invalid value
    const playlistWithInvalidStatus = new Playlist({
  ...
  it('should find playlists by text search on title and description', async () => {
    ...
    // Test text search
    const guitarResults = await Playlist.find(
      { $text: { $search: 'guitar' } },
  ...
  it('should find playlists by channelId', async () => {
    ...
    const anotherChannel = new Channel({
    ...
    const channelPlaylists = await Playlist.find({ channelId: testChannelId });
    ...
    const anotherChannelPlaylists = await Playlist.find({ channelId: savedAnotherChannel._id });
