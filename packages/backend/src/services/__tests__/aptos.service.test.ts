...
import { WalletAuthRequest, AccountLinkRequest } from '@yt2aptos/shared';
...
describe('AptosService', () => {
  ...
  describe('generateNonce', () => {
    it('should generate a valid nonce for a wallet address', async () => {
      ...
      const user = await UserModel.findOne({ walletAddress: testWalletAddress.toLowerCase() });
  ...
  describe('authenticateWithWallet', () => {
    beforeEach(async () => {
      ...
      jest.spyOn(aptosService as any, 'verifySignature').mockResolvedValue(true);
      
      // Mock the getWalletProperties method
      jest.spyOn(aptosService, 'getWalletProperties').mockResolvedValue({
        hasSpecialNft: true,
        tokenBalance: 5000,
    ...
    it('should create a new user when wallet is not found', async () => {
      ...
      const user = await UserModel.findOne({ walletAddress: newWalletAddress.toLowerCase() });
      expect(user).toBeDefined();
    ...
    it('should reject authentication with invalid signature', async () => {
      ...
      (aptosService as any).verifySignature.mockResolvedValue(false);
      ...
      await expect(aptosService.authenticateWithWallet(authRequest))
  ...
  describe('linkWalletToAccount', () => {
    ...
    it('should link a wallet to an existing account', async () => {
      ...
      const result = await aptosService.linkWalletToAccount(testUserId, linkRequest);
    ...
    it('should reject linking if wallet is already linked to another account', async () => {
      ...
      await expect(aptosService.linkWalletToAccount(testUserId, linkRequest))
  ...
  describe('verifySignature', () => {
    it('should verify a valid signature format', async () => {
      ...
      const isValid = await (aptosService as any).verifySignature(
        testWalletAddress,
        testSignature,
        message
      );
