...
describe('DownloadController', () => {
  ...
  describe('getUserJobs', () => {
    it('should return all user jobs', async () => {
      ...
      (DownloadJob.find as jest.Mock).mockImplementation(() => ({
        sort: sortMock,
      ...
      // Assert
      expect(DownloadJob.find).toHaveBeenCalledWith({ userId: 'test-user-id' });
    ...
    it('should handle unauthorized requests', async () => {
      ...
      // Assert
      expect(DownloadJob.find).not.toHaveBeenCalled();
  ...
  describe('getJob', () => {
    it('should return job details for valid ID', async () => {
      ...
      (DownloadJob.findOne as jest.Mock).mockImplementation(() => ({
        populate: populateMock,
        exec: execMock
      }));
      ...
      // Assert
      expect(DownloadJob.findOne).toHaveBeenCalledWith({
        _id: 'job-1',
        userId: 'test-user-id'
      });
    ...
    it('should handle unauthorized requests', async () => {
      ...
      // Assert
      expect(DownloadJob.findOne).not.toHaveBeenCalled();
      expect(mockResponse.status).toHaveBeenCalledWith(401);
      expect(responseJson).toHaveBeenCalledWith({ error: 'Unauthorized' });
    });
  ...
  describe('createJob', () => {
    it('should create a download job successfully', async () => {
      ...
      (Video.findById as jest.Mock).mockResolvedValue({
        _id: videoId,
        title: 'Test Video'
      });
      
      // Mock DownloadJob.findOne to return null (no existing job)
      (DownloadJob.findOne as jest.Mock).mockResolvedValue(null);
      
      // Mock downloadQueueService.addJob
      (downloadQueueService.addJob as jest.Mock).mockResolvedValue(newJob);
      ...
      // Assert
      expect(Video.findById).toHaveBeenCalledWith(videoId);
      expect(DownloadJob.findOne).toHaveBeenCalledWith({
        videoId,
        userId,
        status: { $in: ['queued', 'processing', 'paused'] }
      });
      expect(downloadQueueService.addJob).toHaveBeenCalledWith(
        videoId,
        userId,
        {
          priority: JobPriority.HIGH,
    ...
    it('should handle missing video ID', async () => {
      ...
      mockRequest.body = { priority: JobPriority.NORMAL };
    ...
    it('should handle video not found', async () => {
      // Setup
      (Video.findById as jest.Mock).mockResolvedValue(null);
    ...
    it('should handle existing job', async () => {
      ...
      (DownloadJob.findOne as jest.Mock).mockResolvedValue(existingJob);
      ...
      // Assert
      expect(downloadQueueService.addJob).not.toHaveBeenCalled();
    ...
    it('should handle errors with next()', async () => {
      // Setup
      const mockError = new Error('Database error');
      (Video.findById as jest.Mock).mockImplementation(() => {
        throw mockError;
      });
  ...
  describe('pauseJob', () => {
    it('should pause a job successfully', async () => {
      ...
      (DownloadJob.findOne as jest.Mock).mockResolvedValue(mockJob);
      
      // Mock downloadQueueService.pauseJob
      (downloadQueueService.pauseJob as jest.Mock).mockResolvedValue(true);
      ...
      expect(downloadQueueService.pauseJob).toHaveBeenCalledWith(jobId);
    ...
    it('should handle job not found', async () => {
      ...
      // Assert
      expect(downloadQueueService.pauseJob).not.toHaveBeenCalled();
      expect(mockResponse.status).toHaveBeenCalledWith(404);
      expect(responseJson).toHaveBeenCalledWith({ error: 'Job not found' });
    });
    ...
    it('should handle pause operation failure', async () => {
      ...
      (DownloadJob.findOne as jest.Mock).mockResolvedValue(mockJob);
      (downloadQueueService.pauseJob as jest.Mock).mockResolvedValue(false);
  ...
  describe('getQueueStats', () => {
    it('should return queue stats', async () => {
      ...
      (downloadQueueService.getQueueStats as jest.Mock).mockResolvedValue(mockStats);
      
      // Execute
      await downloadController.getQueueStats(mockRequest, mockResponse, mockNext);
      
      // Assert
      expect(downloadQueueService.getQueueStats).toHaveBeenCalled();
    ...
    it('should handle errors with next()', async () => {
      ...
      (downloadQueueService.getQueueStats as jest.Mock).mockRejectedValue(mockError);
  ...
  describe('getQualityProfiles', () => {
    it('should return available quality profiles', async () => {
      ...
      (ytdlpService.getConfig as jest.Mock).mockReturnValue({
      ...
      // Assert
      expect(ytdlpService.getConfig).toHaveBeenCalled();
    ...
    it('should handle errors with next()', async () => {
      // Setup
      const mockError = new Error('Service error');
      (ytdlpService.getConfig as jest.Mock).mockImplementation(() => {
        throw mockError;
      });
  ...
  describe('getVideoInfo', () => {
    it('should return video info for valid URL', async () => {
      ...
      (ytdlpService.getVideoInfo as jest.Mock).mockResolvedValue(mockVideoInfo);
      ...
      // Assert
      expect(ytdlpService.getVideoInfo).toHaveBeenCalledWith('https://youtube.com/watch?v=video123');
    ...
    it('should handle missing URL', async () => {
      ...
      // Assert
      expect(ytdlpService.getVideoInfo).not.toHaveBeenCalled();
      expect(mockResponse.status).toHaveBeenCalledWith(400);
    ...
    it('should handle errors with next()', async () => {
      // Setup
      const mockError = new Error('Service error');
      (ytdlpService.getVideoInfo as jest.Mock).mockRejectedValue(mockError);
