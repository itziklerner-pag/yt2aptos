...
describe('Channel Model', () => {
    ...
    it('should require youtubeId, name, and createdBy fields', async () => {
        ...
        const channelWithoutYoutubeId = new channel_model_1.Channel({
            name: channelData.name,
            createdBy: testUserId,
        });
        // Test without name
        const channelWithoutName = new channel_model_1.Channel({
            youtubeId: 'UC67890fghij',
            createdBy: testUserId,
        });
        // Test without createdBy
        const channelWithoutCreatedBy = new channel_model_1.Channel({
            youtubeId: 'UC67890fghij',
            name: channelData.name,
        });
        // Validate each case
    ...
    it('should not allow duplicate youtubeId', async () => {
        ...
        await new channel_model_1.Channel({
            youtubeId: 'UC98765zyxwv',
            name: 'Original Channel',
            createdBy: testUserId,
        }).save();
        // Try to create another channel with the same youtubeId
        const duplicateChannel = new channel_model_1.Channel({
            youtubeId: 'UC98765zyxwv',
    ...
    it('should set default values correctly', async () => {
        const channelWithDefaults = new channel_model_1.Channel({
    ...
    it('should only allow valid enum values for archiveStatus', async () => {
        ...
        // Test invalid value
        const channelWithInvalidStatus = new channel_model_1.Channel({
    ...
    it('should find channels by text search on name and description', async () => {
        ...
        // Test text search
        const cookingResults = await channel_model_1.Channel.find({ $text: { $search: 'cooking' } }, { score: { $meta: 'textScore' } }).sort({ score: { $meta: 'textScore' } });
    ...
    it('should find channels by archive status', async () => {
        ...
        // Query by isArchived
        const archivedChannels = await channel_model_1.Channel.find({ isArchived: true });
        expect(archivedChannels.length).toBe(2);
        // Query by archiveStatus
        const completeChannels = await channel_model_1.Channel.find({ archiveStatus: 'complete' });
    ...
    it('should find channels by createdBy user', async () => {
        ...
        const userChannels = await channel_model_1.Channel.find({ createdBy: testUserId });
        ...
        const anotherUserChannels = await channel_model_1.Channel.find({ createdBy: savedAnotherUser._id });
        expect(anotherUserChannels.length).toBe(1);
        expect(anotherUserChannels[0].youtubeId).toBe('UC_another_user');
    });
});
//# sourceMappingURL=channel.model.test.js.map