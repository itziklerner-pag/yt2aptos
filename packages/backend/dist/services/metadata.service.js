...
class MetadataService {
     ...
     */
    constructor() {
        (0, logger_1.logInfo)('MetadataService initialized');
    }
    /**
     ...
     */
    async extractMetadata(videoId, options) {
        const fullOptions = { ...this.DEFAULT_OPTIONS, ...options };
        try {
            (0, logger_1.logInfo)(`Extracting metadata for video: ${videoId}`);
            // Collect metadata from multiple sources for comprehensive coverage
            const sources = [];
            let rawMetadata = {};
            // 1. Get metadata from YouTube API
            try {
                const youtubeMetadata = await youtube_service_1.youtubeService.getVideo(videoId);
                sources.push('youtube-api');
                rawMetadata.youtubeApi = youtubeMetadata;
            }
            catch (error) {
                (0, logger_1.logWarning)(`Failed to get metadata from YouTube API for ${videoId}:`, error);
            }
            // 2. Get metadata from yt-dlp
            try {
                const ytdlpMetadata = await ytdlp_service_1.ytdlpService.getVideoInfo(`https://www.youtube.com/watch?v=${videoId}`);
                sources.push('yt-dlp');
                rawMetadata.ytdlp = ytdlpMetadata;
            }
            catch (error) {
                (0, logger_1.logWarning)(`Failed to get metadata from yt-dlp for ${videoId}:`, error);
            }
            // 3. Look for existing metadata file
            const videoDoc = await mongoose_1.default.model('Video').findOne({ youtubeId: videoId });
            if (videoDoc?.metadataPath) {
                try {
                    ...
                    (0, logger_1.logWarning)(`Failed to read existing metadata file for ${videoId}:`, error);
                }
            }
            ...
            if (fullOptions.extractThumbnails && videoDoc) {
                try {
                    const thumbnailPath = await this.processThumbnail(videoId, standardizedMetadata, videoDoc, fullOptions.thumbnailQuality);
                    if (thumbnailPath) {
                    ...
                    (0, logger_1.logWarning)(`Failed to process thumbnail for ${videoId}:`, error);
                }
            }
            // Process subtitles if requested
            if (fullOptions.extractSubtitles && videoDoc) {
                try {
                    const subtitlePaths = await this.processSubtitles(videoId, standardizedMetadata, videoDoc, fullOptions.subtitleLanguages);
                    if (subtitlePaths && subtitlePaths.length > 0) {
                        result.subtitlePaths = subtitlePaths;
                    }
                }
                catch (error) {
                    (0, logger_1.logWarning)(`Failed to process subtitles for ${videoId}:`, error);
                }
            }
            ...
            if (fullOptions.persistToStorage && videoDoc) {
                try {
                    ...
                    await mongoose_1.default.model('Video').findByIdAndUpdate(videoDoc._id, {
                        metadataPath: storagePath
                    });
                }
                catch (error) {
                    (0, logger_1.logWarning)(`Failed to persist metadata for ${videoId}:`, error);
                }
            }
            return result;
        }
        catch (error) {
            (0, logger_1.logError)(`Error extracting metadata for ${videoId}:`, error);
            return {
                success: false,
                errorMessage: `Error extracting metadata: ${error.message}`
            };
        }
    }
    /**
     ...
     */
    standardizeMetadata(rawMetadata, sources, videoId) {
        ...
        if (rawMetadata.ytdlp) {
            ...
            if (ytdlp.thumbnails && (!metadata.thumbnails || Object.keys(metadata.thumbnails).length === 0)) {
                ...
                for (const thumbnail of ytdlp.thumbnails) {
                    ...
                    if (thumbnail.preference && thumbnail.preference > 0) {
                        ...
                        if (thumbnail.preference >= 10)
                            quality = 'maxres';
                        else if (thumbnail.preference >= 5)
                            quality = 'high';
                        else if (thumbnail.preference >= 0)
                            quality = 'medium';
                        else
                            quality = 'default';
                    }
            ...
            if (ytdlp.formats && ytdlp.formats.length > 0) {
                const bestFormat = ytdlp.formats.find((f) => f.format_id === ytdlp.format_id) || ytdlp.formats[0];
            ...
            if (ytdlp.chapters && ytdlp.chapters.length > 0) {
                metadata.chapters = ytdlp.chapters.map((chapter) => ({
                    title: chapter.title,
        ...
        if (rawMetadata.existingFile) {
            ...
            if (!metadata.duration && existing.duration)
                metadata.duration = existing.duration;
            if (!metadata.viewCount && existing.viewCount)
                metadata.viewCount = existing.viewCount;
            if (!metadata.fileSize && existing.fileSize)
                metadata.fileSize = existing.fileSize;
            if (!metadata.filePath && existing.filePath)
                metadata.filePath = existing.filePath;
            if (!metadata.format && existing.format)
                metadata.format = existing.format;
     ...
     */
    async processThumbnail(videoId, metadata, videoDoc, quality) {
        ...
        if (videoDoc.thumbnailUrl && videoDoc.thumbnailUrl.startsWith('http')) {
            try {
                ...
                if (videoDoc.filePath) {
                    const relativePath = path.relative(storage_service_1.storageService.getProvider().getBasePath(), storagePath);
                    const publicUrl = await storage_service_1.storageService.getPublicUrl(relativePath);
        ...
        try {
            ...
            (0, logger_1.logWarning)(`Failed to download thumbnail for ${videoId}:`, error);
            return undefined;
        }
    }
    /**
     ...
     */
    async processSubtitles(videoId, metadata, videoDoc, languages) {
        ...
        if (videoDoc.hasSubtitles &&
            ...
            if (languages) {
                const missingLanguages = languages.filter(lang => !videoDoc.subtitleLanguages?.includes(lang));
        ...
        try {
            ...
            const result = await ytdlp_service_1.ytdlpService.downloadVideo(`https://www.youtube.com/watch?v=${videoId}`, outputDir, 'standard', // Use standard profile
            options, `subtitle-${Date.now()}`, // Generate a unique job ID
            videoDoc._id ? videoDoc._id.toString() : videoDoc.youtubeId, 'system');
            if (!result.success) {
     ...
     */
    async persistMetadata(videoId, standardizedMetadata, rawMetadata, videoDoc) {
     ...
     */
    async searchVideosByMetadata(query, options) {
        const limit = options?.limit || 20;
        const offset = options?.offset || 0;
        const sortBy = options?.sortBy || 'publishedAt';
        const sortDirection = options?.sortDirection || 'desc';
        const sortOptions = { [sortBy]: sortDirection === 'asc' ? 1 : -1 };
        ...
        // Execute the search
        try {
            const Video = mongoose_1.default.model('Video');
            ...
            (0, logger_1.logError)(`Error searching videos by metadata:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async getDetailedMetadata(videoId, includeRaw = false) {
        try {
            const videoDoc = await mongoose_1.default.model('Video').findById(videoId);
            if (!videoDoc) {
            ...
            (0, logger_1.logError)(`Error getting detailed metadata for ${videoId}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async batchProcessMetadata(videoIds, options) {
        const results = {};
        ...
        for (const videoId of videoIds) {
            try {
                ...
                (0, logger_1.logError)(`Error processing metadata for ${videoId}:`, error);
                results[videoId] = {
                    success: false,
                    errorMessage: `Error processing metadata: ${error.message}`
                };
            }
        }
        return results;
     ...
     * @returns Metadata extraction result
     */
    async extractMetadataFromFile(filePath, videoId, options) {
        try {
            ...
            const mediaMetadata = await post_processing_service_1.postProcessingService.extractMetadata(filePath);
            ...
            if (videoId) {
                try {
                    ...
                    (0, logger_1.logWarning)(`Failed to get YouTube metadata for ${videoId}:`, error);
                }
            }
            // Extract media streams information
            if (mediaMetadata.streams) {
                // Find video and audio streams
                const videoStream = mediaMetadata.streams.find((s) => s.codec_type === 'video');
                const audioStream = mediaMetadata.streams.find((s) => s.codec_type === 'audio');
                if (videoStream) {
            ...
            if (options?.extractThumbnails) {
                try {
                    const thumbnailPath = await post_processing_service_1.postProcessingService.extractThumbnail(filePath, path.dirname(filePath));
                    if (thumbnailPath) {
                        standardizedMetadata.thumbnails = {
                            ...standardizedMetadata.thumbnails,
                            local: thumbnailPath
                    ...
                    (0, logger_1.logWarning)(`Failed to extract thumbnail from ${filePath}:`, error);
                }
            }
            // Extract chapters if available
            if (options?.extractChapters && mediaMetadata.chapters) {
                standardizedMetadata.chapters = mediaMetadata.chapters.map((chapter) => ({
            ...
            if (!videoId) {
                ...
                const videoDoc = await mongoose_1.default.model('Video').findOne({ filePath });
                if (!videoDoc) {
            ...
            if (options?.persistToStorage) {
                try {
                    const storagePath = path.join(path.dirname(filePath), `${path.basename(filePath, path.extname(filePath))}-metadata.json`);
                    const metadataContent = {
                        standardized: standardizedMetadata,
                        raw: {
                            file: mediaMetadata,
                    ...
                    (0, logger_1.logWarning)(`Failed to persist metadata for ${filePath}:`, error);
                }
            }
            return {
                success: true,
            ...
            (0, logger_1.logError)(`Error extracting metadata from file ${filePath}:`, error);
            return {
                success: false,
                errorMessage: `Error extracting metadata from file: ${error.message}`
            };
        }
    }
    /**
     ...
     */
    convertIsoDurationToSeconds(isoDuration) {
        if (!isoDuration)
            return undefined;
        // Format: PT#H#M#S
        ...
        if (seconds)
            totalSeconds += parseInt(seconds[1], 10);
        return totalSeconds;
    }
}
